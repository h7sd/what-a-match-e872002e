...
export function UserBadgesList({ userBadges, userId }: UserBadgesListProps) {
  ...
  const handleToggle = async (userBadgeId: string, currentEnabled: boolean, isLocked: boolean) => {
    if (isLocked) {
      toast({ 
        title: 'Badge is locked', 
        description: 'This badge has been locked by an administrator.',
        variant: 'destructive' 
      });
      return;
    }
  ...
  const lockedBadges = userBadges.filter(ub => ub.is_locked);
  const unlockedBadges = userBadges.filter(ub => !ub.is_locked);
      ...
      {unlockedBadges.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {unlockedBadges.map((ub) => {
            ...
            const isEnabled = ub.is_enabled !== false;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {updating === ub.id ? (
                      ...
                      <Switch
                        checked={isEnabled}
                        onCheckedChange={() => handleToggle(ub.id, isEnabled, false)}
                      />
                    )}
                  </div>
                </div>
