...
Deno.serve(async (req) => {
  ...
  const maybeUid = Number.parseInt(username, 10);
  ...
  const { data: byName, error: nameErr } = await supabase
    .from("profiles")
    .select(
      "username, display_name, bio, avatar_url, og_title, og_description, og_image_url, og_icon_url, uid_number",
    )
  ...
  if (byName) {
    profile = byName;
  } else if (!Number.isNaN(maybeUid) && maybeUid > 0) {
  ...
  const ogDescription =
    ...
    profile.bio ||
    `Check out ${profile.display_name || profile.username}'s profile on UserVault`;
  ...
  const baseOgImage =
    ...
    profile.avatar_url ||
    "https://what-a-match.lovable.app/og-image.png";
  ...
  const ogIcon =
    profile.og_icon_url ||
    "https://storage.googleapis.com/gpt-engineer-file-uploads/N7OIoQRjNPSXaLFdJjQDPkdaXHs1/uploads/1769473434323-UserVault%204%20(1).png";
