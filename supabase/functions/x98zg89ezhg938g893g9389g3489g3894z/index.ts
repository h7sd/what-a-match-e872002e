...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[APPROVE] Processing request: ${requestId}`);
    ...
    // Optional: load profile username for logs (not required for approval)
    ...
    console.log(
      `[APPROVE] Found request for user: ${request.user_id}${profile?.username ? ` (@${profile.username})` : ""}`
    );
    ...
    // Use original values
    const finalName = request.badge_name;
    const finalDescription = request.badge_description;
    const finalColor = request.badge_color;
    const finalIconUrl = request.badge_icon_url;
    ...
    console.log(`[APPROVE] Created badge: ${badge.id}`);
